...
async function audit(uid:string,action:string,type:string,id:string,metadata:any={},beforeState:any=null,afterState:any=null){try{await db.from("audit_log").insert({actor_user_id:uid,action,entity_type:type,entity_id:id,reason:"RPK document/accounting action",metadata,before_state:beforeState,after_state:afterState})}catch{}}
...
Deno.serve(async req=>{
 ...
 const meta=(x:any={})=>({workspace_id:w.id,actor_role:role,...x});
 ...
 if(action==="register_upload"){
  ...
  const type=DOC_TYPES.has(safe(b.document_type,30))?safe(b.document_type,30):"other";const total=num(b.total);if(total!=null&&total<0)return out(400,{ok:false,error:"negative_total_not_allowed"},origin);
  ...
  if(error)return out(500,{ok:false,error:"document_register_failed",detail:error.message},origin);await audit(u.user.id,"rpk.document_uploaded","rpk_document",data.id,meta({file_ext:e,document_type:type,client_id:clientId,deal_id:dealId}));return out(200,{ok:true,document:data},origin);
 }

 if(action==="download"){
  const id=safe(b.document_id,80);const {data:d}=await db.from("rpk_documents").select("id,storage_path,original_name").eq("workspace_id",w.id).eq("id",id).maybeSingle();if(!d?.storage_path)return out(404,{ok:false,error:"document_file_not_found"},origin);const {data,error}=await db.storage.from("rpk-documents").createSignedUrl(d.storage_path,900,{download:d.original_name||true});if(error||!data)return out(500,{ok:false,error:"signed_download_failed",detail:error?.message},origin);await audit(u.user.id,"rpk.document_downloaded","rpk_document",id,meta({original_name:d.original_name}));return out(200,{ok:true,url:data.signedUrl,expires_in:900},origin);
 }
 ...
 if(action==="create_accounting_request"){
  ...
  await db.from("rpk_communication_events").insert({workspace_id:w.id,accounting_thread_id:data.id,deal_id:dealId,client_id:clientId,direction:"out",channel:channel==="other"?"manual":channel,actor_side:"rpk",subject,message_excerpt:safe(b.notes,1000)||"Запрос бухгалтеру создан",occurred_at:now,metadata:{manual:true,created_by:u.user.id}});await audit(u.user.id,"rpk.accounting_request_created","rpk_accounting_thread",data.id,meta({request_type:type,channel,client_id:clientId,deal_id:dealId}));return out(200,{ok:true,thread:data},origin);
 }

 if(action==="update_accounting_request"){
  if(!MANAGE_ROLES.has(role))return out(403,{ok:false,error:"manager_role_required"},origin);const id=safe(b.thread_id,80),status=safe(b.status,40);if(!THREAD_STATUSES.has(status))return out(400,{ok:false,error:"invalid_accounting_status"},origin);const {data:old}=await db.from("rpk_accounting_threads").select("*").eq("workspace_id",w.id).eq("id",id).maybeSingle();if(!old)return out(404,{ok:false,error:"accounting_thread_not_found"},origin);const patch:any={status,updated_at:new Date().toISOString()};if(status==="received")patch.received_at=new Date().toISOString();if(["paid","closed"].includes(status))patch.completed_at=new Date().toISOString();if(b.result_document_id!==undefined)patch.result_document_id=safe(b.result_document_id,80)||null;const {data,error}=await db.from("rpk_accounting_threads").update(patch).eq("workspace_id",w.id).eq("id",id).select("*").maybeSingle();if(error||!data)return out(404,{ok:false,error:"accounting_thread_not_found",detail:error?.message},origin);await audit(u.user.id,"rpk.accounting_request_updated","rpk_accounting_thread",id,meta({status}),old,data);return out(200,{ok:true,thread:data},origin);
 }

 if(action==="log_communication"){
  if(!MANAGE_ROLES.has(role))return out(403,{ok:false,error:"manager_role_required"},origin);const threadId=safe(b.thread_id,80)||null;if(threadId){const {data:t}=await db.from("rpk_accounting_threads").select("id").eq("workspace_id",w.id).eq("id",threadId).maybeSingle();if(!t)return out(404,{ok:false,error:"accounting_thread_not_found"},origin)}const channel=CHANNELS.has(safe(b.channel,30))?safe(b.channel,30):"manual";const direction=["in","out","internal"].includes(safe(b.direction,20))?safe(b.direction,20):"internal";const actor=["rpk","accountant","client","supplier","system"].includes(safe(b.actor_side,30))?safe(b.actor_side,30):"rpk";const {data,error}=await db.from("rpk_communication_events").insert({workspace_id:w.id,accounting_thread_id:threadId,direction,channel:channel==="other"?"manual":channel,actor_side:actor,subject:safe(b.subject,300)||null,message_excerpt:safe(b.message_excerpt,1500)||null,occurred_at:b.occurred_at?new Date(b.occurred_at).toISOString():new Date().toISOString(),metadata:{manual:true,created_by:u.user.id}}).select("*").single();if(error)return out(500,{ok:false,error:"communication_log_failed",detail:error.message},origin);await audit(u.user.id,"rpk.communication_logged","rpk_communication_event",data.id,meta({thread_id:threadId,channel,direction,actor_side:actor}));return out(200,{ok:true,event:data},origin);
 }

 if(action==="register_mailbox"){
  if(!MANAGE_ROLES.has(role))return out(403,{ok:false,error:"manager_role_required"},origin);const purpose=["company","accountant","other"].includes(safe(b.purpose,30))?safe(b.purpose,30):"other";const provider=["gmail","imap","other"].includes(safe(b.provider,30))?safe(b.provider,30):"other";const email=safe(b.email_address,240).toLowerCase();if(!email.includes("@"))return out(400,{ok:false,error:"valid_email_required"},origin);const {data,error}=await db.from("rpk_mailbox_connections").upsert({workspace_id:w.id,purpose,provider,email_address:email,status:"pending",updated_at:new Date().toISOString()},{onConflict:"workspace_id,purpose,email_address"}).select("id,purpose,provider,email_address,status,created_at,updated_at").single();if(error)return out(500,{ok:false,error:"mailbox_register_failed",detail:error.message},origin);await audit(u.user.id,"rpk.mailbox_registered","rpk_mailbox_connection",data.id,meta({purpose,provider,email_address:email}));return out(200,{ok:true,mailbox:data,note:"Mailbox registered as pending. OAuth/IMAP authorization is a separate credential step; no password is stored here."},origin);
 }
