...
function renderDetail(d,maxSnap,waSnap){
  const i=d.instance||{},r=i.readiness||{},f=i.features||{},admin=role==='admin',o=d.order||{},fr=d.factory||{};
  ...
  $('#leadbotBody').innerHTML=`<div class="toolbar"><div><p class="eyebrow">SELECTED INSTANCE</p><h2 style="margin:.25rem 0">ORD-${String(o.order_no||selectedOrder).padStart(6,'0')} · ${esc(i.business_name)}</h2><div>${channelSummary}</div></div><button id="fleetDetailRefresh" class="ghost">Обновить экземпляр</button></div><div class="hero-grid" style="margin-top:14px"><article class="panel metric"><small>INSTANCE</small><strong>${esc(String(i.status||'—').toUpperCase())}</strong><span>${esc(i.public_slug||'')}</span></article><article class="panel metric"><small>CORE</small><strong class="${r.core_ready?'on':'off'}">${r.core_ready?'READY':'WAITING'}</strong><span>хотя бы один канал готов</span></article><article class="panel metric"><small>DELIVERY</small><strong class="${r.delivery_ready?'on':'off'}">${r.delivery_ready?'READY':'BLOCKED'}</strong><span>заказанные модули</span></article><article class="panel metric"><small>ORDER</small><strong>${money(o.quoted_amount,o.currency)}</strong><span>${esc(fr.status||'—')} · ${esc(o.status||'—')}</span></article></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px;margin-top:16px">${telegram}${maxBlock}${waBlock}</div>${advanced}<div class="toolbar" style="margin-top:22px"><h3>MiniCRM · все каналы</h3><span class="muted">outbox problems: ${failed}</span></div><section class="cards">${leads}</section>`;
  bindDetail(i);
}
