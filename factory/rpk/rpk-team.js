...
function human(a){
  const m={
    'rpk.member_added':'Добавил сотрудника',
    'rpk.member_role_changed':'Изменил роль сотрудника',
    'rpk.member_disabled':'Отключил сотрудника',
    'rpk.deal_created':'Создал сделку',
    'rpk.deal_updated':'Изменил сделку',
    'rpk.cash_entry_created':'Добавил приход / расход',
    'rpk.document_draft_created':'Создал черновик документа',
    'rpk.document_uploaded':'Загрузил документ',
    'rpk.document_cloned':'Создал новый документ из старого',
    'rpk.accounting_request_created':'Создал запрос бухгалтеру',
    'rpk.accounting_request_updated':'Изменил статус запроса бухгалтеру',
    'rpk.module_toggled':'Изменил набор модулей'
  };
  if(m[a])return m[a];
...
async function loadTeam(){
  ...
  try{
    const x=await teamApi('snapshot',{limit:150});
    ...
    const logs=(x.activity||[]).filter(v=>v.reason==='RPK live gateway'||v.reason==='RPK team action'||String(v.action||'').startsWith('rpk.'));
